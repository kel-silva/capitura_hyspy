...
async function obterIPPublicoReal(req) {
  // código original seu, não alterado (preservado para brevidade)
  try {
    ...
    // Buscar IP público externo se IP detectado for privado
    if (isPrivateIP) {
      const servicos = [
...
// ========== DASHBOARD HTML DINÂMICO COM LAYOUT DAS IMAGENS ==========
app.get('/dashboard', autenticar, (req, res) => {
  // Código da rota dashboard, layout idêntico às imagens com botões funcionando
...
<body>
  ...
  <script>
    ...
    // Atualizar a cada 15 segundos automaticamente
